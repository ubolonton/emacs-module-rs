...
Trimmed from the Emacs 30.2 header to expose emacs_env_28 as the
top-level emacs_env.  env_29 and env_30 structs removed.
*/
...
/* Compat alias: Emacs 28 renamed emacs_finalizer_function to emacs_finalizer.
   Existing code (e.g. user_ptr.rs) imports the old name.  */
typedef emacs_finalizer emacs_finalizer_function;
...
struct emacs_env_25
{
  ...
  emacs_finalizer (*get_user_finalizer) (emacs_env *env, emacs_value uptr)
    EMACS_ATTRIBUTE_NONNULL(1);
...
struct emacs_env_28
{
  ...
  emacs_finalizer (*get_function_finalizer) (emacs_env *env, emacs_value arg)
    EMACS_ATTRIBUTE_NONNULL (1);
